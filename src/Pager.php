 ...
 *
 * @author HyacinthTechnology
 */
class Pager {
    //put your code here
    public $AbsolutePage = 0;
    public $PageCount = 1;
    public $Size = 10;
    public $Prefix = "?page=";
    public $Suffix = "";
    ...
    public function GetTotal($cssClass = 'total') {
        ...
        if($this->PageCount != 0){
            return '<span' . $cssClass . '>' . '|&nbsp;当前页数:'.$this->AbsolutePage . '/' . $this->PageCount . '</span>'."\r\n";;
        }
    }
    public function ToString($ToConfigures) {
        if($ToConfigures == 1){
            return $this->GetFirst() . $this->GetPrev() . $this->GetPageCodes()
                . $this->GetNext() . $this->GetLast() . $this->GetTotal();
        }elseif ($ToConfigures == 2){
            return  $this->GetPrev() . $this->GetPageCodes()
                . $this->GetNext() ;
        }elseif ($ToConfigures == 3){
